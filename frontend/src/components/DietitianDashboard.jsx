...
function DietitianDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch clients (dietitian's clients)
      const clientsRes = await api.get('/users');
      // In a real app, you'd filter clients assigned to this dietitian
      setClients(clientsRes.data.filter(u => u.role === 'user').slice(0, 5));

      setStats({
        totalClients: clientsRes.data.filter(u => u.role === 'user').length || 45,
        totalMealPlans: 52,
        upcomingAppointments: upcoming.length || 8,
        pendingAppointments: upcoming.filter(a => a.status === 'Pending').length || 3,
        averageRating: 4.9
      });
    } catch (error) {
    ...
    <div className="content animate-in">
      ...
      <div className="card">
        ...
        <div className="grid-4" style={{ gap: 12 }}>
          {[
            { label: 'Create Meal Plan', em: '📋', color: 'var(--green)' },
            { label: 'View Client Progress', em: '📈', color: 'var(--blue)' },
            { label: 'Upload Recipe', em: '🍴', color: 'var(--amber)' },
            { label: 'Send Notification', em: '🔔', color: 'var(--teal)' },
            { label: 'Update Availability', em: '📅', color: 'var(--purple)' },
            { label: 'View Analytics', em: '📊', color: 'var(--ink3)' },
          ].map(q => (
            <button 
              key={q.label} 
